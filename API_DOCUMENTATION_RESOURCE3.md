...
数字警员平台 API 接口文档，提供数字警员、Skill、MCP 服务的管理、授权、审核等功能。

### 核心特性

1. **数字警员管理** - 创建、更新、删除、审核数字警员
2. **Skill 管理** - 技能的全生命周期管理（申请上架→两级审核→自动上架）
3. **MCP 服务管理** - MCP 服务的注册、审核、授权（申请上架→两级审核→自动上架）
4. **资源关联** - 数字警员与 Skill/MCP 的关联管理
5. **统一授权** - 支持部门/用户两级授权体系
6. **两级审核** - 部门审核 + 超级管理员审核（自动流转）
7. **简化删除** - 超级管理员/创建者/部门管理员可直接删除
8. **操作日志** - 完整的操作审计追踪
...
```
创建 → 申请上架 → 部门审核 → 超级审核 → 已上架
00      01         02→01     02        TRUE
静默   申请中    (自动)    (自动上架)
```

**部门审核通过**: 自动设置 `super_audit_status='01'` (待超级审核)  
**超级审核通过**: 自动设置 `is_public=TRUE` (自动上架)

#### 重新申请规则

| 当前状态  | 申请人     | 新状态                | 说明         |
| --------- | ---------- | --------------------- | ------------ |
| `03` 拒绝 | 创建者     | dept=`01`, super=`00` | 从头开始     |
| `03` 拒绝 | 部门管理员 | dept=`02`, super=`01` | 继续超级审核 |
...
**接口**: `POST /skill/applyPublish.xhtml`
...
{
  "status": "succeed",
  "message": "上架申请已提交，等待部门管理员审核"
}
```

#### 1-2. 重新申请上架 Skill

**接口**: `POST /skill/reapplyPublish.xhtml`
...
**接口**: `POST /mcp/applyPublish.xhtml`
...
**接口**: `POST /mcp/reapplyPublish.xhtml`
...
**接口**: `POST /officer/applyPublish.xhtml`
...
**接口**: `POST /officer/reapplyPublish.xhtml`
...
1. **简化审核流程** - 部门审核通过自动提交超级审核，超级审核通过自动上架
2. **删除申请删除流程** - 改为直接删除（超级管理员/创建者/部门管理员）
3. **新增申请上架接口** - applyPublish/reapplyPublish
4. **统一状态码** - 00-静默，01-申请中，02-通过，03-拒绝
5. **数据库优化** - 删除删除审核相关字段（delete_reason 等 7 个字段）
6. **代码精简** - 删除申请删除相关接口和方法

### v7.0 优化内容

1. **统一审核流程** - 使用 ResourceAuditManager 统一管理
2. **统一授权流程** - 使用 UnifiedAuthManager 统一授权接口
3. **数据库连接优化** - 使用 db_cursor 上下文管理器
4. **Token 验证优化** - 使用 @require_token 装饰器
